...
public:
	...
	void endProcessing(std::vector<double> &featuresVector, std::vector<string> &resultVectorNames);
};
...
void AppGlcm::initProcessing(int step, int angle, int num_colors) {
	...
	///cout<< nameBD << ", "<< nameFileResults << ", "<< endl;
...
void AppGlcm::offLineProcessing(){
	...
	while (!file.eof()) {
		...
		cout << "nameImage:" << nameImage<<endl;
		cv::Mat src = cv::imread(nameImage, 0);
		//imshow("reading file image", src);
		//waitKey();
...
void AppGlcm::endProcessing(std::vector<double> &resultVector, std::vector<std::string> &resultVectorNames ){
	cout << "endProcessing\n";
	glcm->getFeaturesVector(resultVector);
	glcm->getNameFeaturesVector(resultVectorNames);
	glcm->clear();
	//file.close();
	//fileResults.close();
	//cv::waitKey();
